...
def get_train_valid_dataloader(options):
    if options.tmp_data:
        train_dataset = TmpDataset(options.tmp_data)
        valid_dataset = TmpDataset(options.tmp_data)
        train_dataloader = DataLoader(train_dataset, batch_size=options.batch_size, shuffle=options.data.random_split,
                   num_workers=options.num_workers)
        valid_dataloader = DataLoader(valid_dataset, batch_size=options.batch_size, shuffle=options.data.random_split,
                                      num_workers=options.num_workers)
